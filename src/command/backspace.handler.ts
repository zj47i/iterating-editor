...
import { DomVDomConverter } from "../vdom/dom-vdom-converter.ts";
...
export class BackspaceHandler {
    ...
    handleEmptyTextNode(textNode: Text) {
        ...
        if (previousVSpan) {
            ...
            position(
                this.sync.findDomNodeFrom(previousVSpan).getElement(),
            ...
            if (nextVSpan) {
                position(this.sync.findDomNodeFrom(nextVSpan).getElement(), 0);
            } else {
