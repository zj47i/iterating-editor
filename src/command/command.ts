...
export class Command {
  ...
  constructor(
    ...
    this.target.addEventListener("input", (event) => {
      if (this.compositionStateMachine.getState().getName() !== "IdleState") {
        return;
      }
  ...
  keydown(event: KeyboardEvent) {
    ...
    if (event.key === CommandKeyboardEvent.BACKSPACE) {
      ...
      if (
        currentSelectionState.startContainer.nodeType === Node.TEXT_NODE &&
      ...
      if (currentSelectionState.startContainer.nodeName === "P") {
        if (!(currentSelectionState.startContainer instanceof HTMLElement)) {
          throw new Error("anchorNode is not HTMLElement");
        }
        this.backspaceHandler.handleParagraph(
          DomNode.fromExistingElement(currentSelectionState.startContainer),
          event
        );
      }
    }
    ...
    if (event.key.toUpperCase() === "Z" && event.ctrlKey) {
      const shortcutUndo = ShortcutUndo.getInstance<ShortcutUndo>(this.sync);
      event.preventDefault();
